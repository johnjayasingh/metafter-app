...
/// AHD Tasmania Step 2 — Health conditions and concerns
///
/// API fields:
///   - health_conditions.major_health_conditions
class AhdStep2TasScreen extends StatefulWidget {
  final AhdFlowData flowData;

  const AhdStep2TasScreen({super.key, required this.flowData});

  @override
  State<AhdStep2TasScreen> createState() => _AhdStep2TasScreenState();
}

class _AhdStep2TasScreenState extends State<AhdStep2TasScreen> {
  ...
  void initState() {
    ...
    _healthConditionsController =
        TextEditingController(text: widget.flowData.tasHealthConditions ?? '');
  }
  ...
  AhdFlowData _collectData() {
    return (_returnedFromNext ?? widget.flowData).copyWith(
      tasHealthConditions: _healthConditionsController.text.trim(),
    );
  }
