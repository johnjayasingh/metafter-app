...
/// AHD NT Step 6 — Decision maker preferences
///
/// API fields:
///   - attorney_and_advice.attorney_decision_power_detail
class AhdStep6NtScreen extends StatefulWidget {
  final AhdFlowData flowData;

  const AhdStep6NtScreen({super.key, required this.flowData});

  @override
  State<AhdStep6NtScreen> createState() => _AhdStep6NtScreenState();
}

class _AhdStep6NtScreenState extends State<AhdStep6NtScreen> {
  final _formKey = GlobalKey<FormState>();
  bool _isSubmitting = false;

  late String? _decisionMethod;
  late final TextEditingController _decisionMethodOtherController;

  @override
  void initState() {
    super.initState();
    _decisionMethod = widget.flowData.ntDecisionMethod;
    _decisionMethodOtherController =
        TextEditingController(text: widget.flowData.ntDecisionMethodOther ?? '');
  }

  @override
  void dispose() {
    _decisionMethodOtherController.dispose();
    super.dispose();
  }

  AhdFlowData _collectData() {
    return widget.flowData.copyWith(
      ntDecisionMethod: _decisionMethod,
      ntDecisionMethodOther: _decisionMethodOtherController.text.trim(),
    );
  }

  // ── Enum mapping helpers (from old step 7) ──
  ...
  String _mapMatters(String m) {
    switch (m) {
      case NtMatters.allMatters:
        return AhdMatters.all;
      case NtMatters.personalHealthMatters:
        return AhdMatters.personal;
              ...
              child: SingleChildScrollView(
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('Decision maker preferences',
                          style: AppTextStyles.pageTitle),
                      const SizedBox(height: 24),

                      Text(
                        'How do you prefer your decision maker(s) to make decisions?',
                        style: AppTextStyles.questionTitle,
                      ),
                      const SizedBox(height: 16),
                      RadioListOption(
                        isSelected:
                            _decisionMethod == NtDecisionMethod.severally,
                        title:
                      ...
                      RadioListOption(
                        isSelected:
                            _decisionMethod == NtDecisionMethod.jointly,
                        title: 'Jointly (unanimously)',
                        onTap: () => setState(() =>
                            _decisionMethod = NtDecisionMethod.jointly),
                      ),
                      const SizedBox(height: 12),
                      RadioListOption(
                        isSelected:
                            _decisionMethod == NtDecisionMethod.other,
                        title:
                            'Other or specific circumstances (for example, if one decision maker is out of the Territory I appoint the other)',
                        onTap: () => setState(() =>
                            _decisionMethod = NtDecisionMethod.other),
                      ),
                      if (_decisionMethod == NtDecisionMethod.other) ...[
                        const SizedBox(height: 12),
                        AppTextArea(
                          controller: _decisionMethodOtherController,
