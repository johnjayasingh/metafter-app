...
/// AHD Tasmania Step 7 — Witnessing (medical practitioner + adult witness)
///
/// API fields (in ahd_persons):
///   - WITNESS_MEDICAL_PRACTITIONER: full_name, other.qualification
///   - WITNESS_PERSON: full_name
class AhdStep7TasScreen extends StatefulWidget {
  final AhdFlowData flowData;

  const AhdStep7TasScreen({super.key, required this.flowData});

  @override
  State<AhdStep7TasScreen> createState() => _AhdStep7TasScreenState();
}

class _AhdStep7TasScreenState extends State<AhdStep7TasScreen> {
  ...
  void initState() {
    super.initState();
    final witnesses = widget.flowData.tasWitnesses;

    // Find existing medical practitioner witness
    final medPractitioner = witnesses.cast<AhdAttorneyData?>().firstWhere(
          (w) => w?.isHealthPractitioner == true,
          orElse: () => null,
        );

    // Find existing adult witness (non-medical practitioner)
    final adultWitness = witnesses.cast<AhdAttorneyData?>().firstWhere(
          (w) => w?.isHealthPractitioner != true,
          orElse: () => null,
        );

    _witness1NameController =
        TextEditingController(text: medPractitioner?.fullName ?? '');
    _witness1QualificationController =
        TextEditingController(text: medPractitioner?.qualification ?? '');
    _witness2NameController =
        TextEditingController(text: adultWitness?.fullName ?? '');
  }
  ...
  AhdFlowData _collectData() {
    final witnesses = <AhdAttorneyData>[];

    final w1Name = _witness1NameController.text.trim();
    if (w1Name.isNotEmpty) {
      witnesses.add(AhdAttorneyData(
        id: 'witness_medical_practitioner',
        firstName: w1Name,
        lastName: '',
        isHealthPractitioner: true,
        qualification: _witness1QualificationController.text.trim().isNotEmpty
            ? _witness1QualificationController.text.trim()
            : null,
      ));
    }

    final w2Name = _witness2NameController.text.trim();
    if (w2Name.isNotEmpty) {
      witnesses.add(AhdAttorneyData(
        id: 'witness_person',
        firstName: w2Name,
        lastName: '',
